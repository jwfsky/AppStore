...
//              setViewPagerScrollerSpeed(mViewPager);
//                mViewPager.setCurrentItem(position);
     ...
     * 自定义ViewPager的滚动速度
     */
    private void setViewPagerScrollerSpeed(ViewPager pager) {
        try {
            if (pager != null) {

                Field mScroller = ViewPager.class.getDeclaredField("mScroller");
                mScroller.setAccessible(true);
                FixedSpeedScroller speedScroller = new FixedSpeedScroller(pager.getContext());
                mScroller.set(pager, speedScroller);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
