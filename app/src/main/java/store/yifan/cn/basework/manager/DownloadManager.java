package store.yifan.cn.basework.manager;
...
import store.yifan.cn.basework.bean.AbDownloadMessage;
...
public class DownloadManager<T> {
	...
	private Map<Long, T> mDownloadMap = new ConcurrentHashMap<Long, T>();
	...
	/** 当下载状态发送改变的时候回调 */
	public void notifyDownloadStateChanged(T info) {
		synchronized (mObservers) {
	...
	/** 当下载进度发送改变的时候回调 */
	public void notifyDownloadProgressed(T info) {
		synchronized (mObservers) {
			/*for (DownloadObserver observer : mObservers) {
	...
	/** 下载，需要传入一个appInfo对象 */
	public synchronized void download(AbDownloadMessage appInfo) {
		// 先判断是否有这个app的下载信息
		/*T info = mDownloadMap.get(appInfo.getId());
		if (info == null) {// 如果没有，则根据appInfo创建一个新的下载信息
			info = T.clone(appInfo);
	...
	/** 暂停下载 */
	public synchronized void pause(AbDownloadMessage appInfo) {
		/*stopDownload(appInfo);
		T info = mDownloadMap.get(appInfo.getId());// 找出下载信息
		if (info != null) {// 修改下载状态
	...
	public synchronized void cancel(AbDownloadMessage appInfo) {
		stopDownload(appInfo);
	...
	/** 安装应用 */
	public synchronized void install(AbDownloadMessage appInfo) {
		stopDownload(appInfo);
		/*DownloadInfo info = mDownloadMap.get(appInfo.getId());// 找出下载信息
	...
	/** 启动应用，启动应用是最后一个 */
	public synchronized void open(AbDownloadMessage appInfo) {
		/*try {
	...
	private void stopDownload(AbDownloadMessage appInfo) {
	...
	/** 获取下载信息 */
	public synchronized T getDownloadInfo(long id) {
	...
	public interface DownloadObserver {
		...
		/*public void onDownloadStateChanged(T info);

		public void onDownloadProgressed(T info);*/
	}
}
